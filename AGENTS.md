...
Каждый `.diff` в `patches/common/` – это `diff` относительно оригинального репо opencode. Применяются через `patch -p1`. Порядок применения – алфавитный (по имени файла).
